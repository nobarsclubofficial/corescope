...
const SCRIPT = path.resolve(REPO_ROOT, 'scripts/check-xss-sinks.sh');
const FIXTURE_DIR = path.resolve(REPO_ROOT, 'testdata/preflight-xss');
...
for (const c of cases) {
  ...
  const tests = (c.tests || []).map(t => path.relative(REPO_ROOT, path.join(FIXTURE_DIR, t)));
